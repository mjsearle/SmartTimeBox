...
{
   if ( currentmode == 2 && ResolveTimeinSeconds(yellowtime) > (ResolveTimeinSeconds(greentime)+60) )
   {
      DecrementLightTime(timecolour);
   }
   if ( currentmode == 3 && ResolveTimeinSeconds(redtime) > (ResolveTimeinSeconds(yellowtime)+60) )
   {
      DecrementLightTime(timecolour);
   }  
   if ( currentmode == 4 )   { DecrementLightTime(timecolour); }     
   if ( currentmode == 5 && ResolveTimeinSeconds(yellowtime) > (ResolveTimeinSeconds(greentime)+1) )
   {
      DecrementLightTime(timecolour);
   }
   if ( currentmode == 6 && ResolveTimeinSeconds(redtime) > (ResolveTimeinSeconds(yellowtime)+1) )
   {
      DecrementLightTime(timecolour);
   }    
} 
...
void TimeColourWrite_Simplified(unsigned char *timecolour)
{
