...
// Threshold times for each of the four presets. Green, Yellow then Red
struct timearray preset1times[3] = {{0, 5, 0, 0} , {5, 5, 0, 0} , {0, 0, 1, 0}};  
struct timearray preset2times[3] = {{0, 0, 5, 0} , {0, 0, 6, 0} , {0, 0, 7, 0}};
struct timearray preset3times[3] = {{0, 0, 7, 0} , {0, 0, 8, 0} , {0, 0, 9, 0}};
struct timearray preset4times[3] = {{0, 0, 3, 1} , {0, 0, 4, 1} , {0, 0, 5, 1}};
...
unsigned char brightness_level = 1;  // Brightness setting has four allowed values. 0 least bright, 5 highest brightness

static volatile unsigned int brightness_on_time = 250;
static volatile unsigned int brightness_off_time = 2750;
 ...
 // If in set green light time mode.
  ...
  { 
    ...
    while( currentmode == 1 )
    {
      ...
      {
        modebut_held = 0;
        currentmode = 4; // Jump to "special" modes 4 and 5 
      }             
    }
  }
   // If in set yellow light time mode.
  if ( currentmode == 2 )
  {
    flash = 0;
    while( currentmode == 2 )
